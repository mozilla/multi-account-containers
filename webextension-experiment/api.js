...
const getCookieStoreIdForTab = function(data, tab) {
  if (data.incognito) {
...
// across all windows.
const TabContext = function TabContext(getDefaults, extension) {
  this.extension = extension;
...
//const getBrowserInfo = getBrowserInfo;
...
const WindowEventManager = function(context, name, event, listener) {
...
function query(extension, queryInfo) {
  let pattern = null;
      ...
      contextualIdentities: {
        hide(cookieStoreId) {
          query(extension, {
            ...
            tabs.forEach((tab) => {
              let tabObj = TabManager.getTab(tab.id, null);
              let gBrowser = tabObj.ownerGlobal.gBrowser;
              gBrowser.hideTab(tabObj);
            });
          }).catch((e) => {
            throw e;
          });
